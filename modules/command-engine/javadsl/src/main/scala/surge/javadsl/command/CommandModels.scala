...
/**
 * Trait for implementing a rejectable command model.  Unlike `AggregateCommandModel` this type of command model may
 * choose to accept or reject commands.
 * @tparam Agg state type
 * @tparam Cmd command type
 * @tparam Rej rejection type
...
trait RejectableAggregateCommandModel[Agg, Cmd, Rej, Evt] extends AggregateCommandModelBase[Agg, Cmd, Rej, Evt] {
  /**
   * Process a command
   * @param ctx the surge context
   * @param aggregate the current aggregate state
   * @param command the command to process
   * @return a CompleteableFuture of Either a rejection or a sequence or events generated by processing the command
   */
   ...
   * @return the some resulting aggregate state or `None` to remove the state.
   */
  def handleEvent(ctx: Context, aggregate: Optional[Agg], event: Evt): Optional[Agg]
