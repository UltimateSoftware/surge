 ...
 * @param eventsToLog CQRS events to log.
 */
 ...
 * @tparam E CQRS Event type
 */

trait AggregateProcessingModel[S, M, +R, E] {
  /**
   * Process a message.  Apply msg to state.  Return either a rejection or a HandledMessageResult which is a
   * resulting state and a sequence of events to be persisted to the CQRS event topic.
   * @param ctx a context object for interacting with the X-engine.
   * @param state the current state of the aggregate
   * @param msg the message
   * @param ec an execution context for generating futures
   * @return the future result of processing the message
   */
   ...
   * @return the future resulting state
   */
