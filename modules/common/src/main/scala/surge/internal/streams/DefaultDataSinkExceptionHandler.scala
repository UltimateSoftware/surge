...
class DefaultDataSinkExceptionHandler[K, V]() extends DataSinkExceptionHandler[K, V] {
  ...
  override def handleException[Meta](key: K, value: V, streamMeta: Meta, exception: Throwable): Unit = {
    log.error(s"An exception was thrown by the event handler for message with metadata [$streamMeta]! " +
      s"The stream will restart and the message will be retried.", exception)
    throw exception
  }
}
