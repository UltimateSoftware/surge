...
class KafkaSubscriptionProviderSpec
  ...
  private def testManualSubscriptionProvider(
    ...
    new ManualOffsetManagementSubscriptionProvider(defaultConfig, topic.name, Subscriptions.topics(topic.name), consumerSettings, businessFlow, offsetManager)(tracer)
  }
